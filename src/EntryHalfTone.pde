public static class EntryHalftone implements Entry {
  ...
  public void setup() {
    ...
    this.context.ui.setCanvasSize(this.context.image.width - (this.context.clip ? (int)(this.context.spacing * 2) : 0),
      this.context.image.height - (this.context.clip ? (int)(this.context.spacing * 2) : 0));
    if (this.context.clip)
      this.context.setDelta(-(int)this.context.spacing, -(int)this.context.spacing);
  }
  ...
  private void addControls() {
    this.context.ui.addCheck("circle", new CheckListener() {
        ...
        public void onChange(boolean value) {
          EntryHalftone.this.context.setCircle(value);
        }
    }, this.context.circle);
    this.context.ui.addCheck("rounded", new CheckListener() {
        @Override
        public void onChange(boolean value) {
    ...
    this.radio = this.context.ui.addRadio(new String[] {"grayscale", "colored", "scatter"}, new RadioListener() {
        @Override
        public void onChange(int value) {
          EntryHalftone.this.context.setScatter(false);
          EntryHalftone.this.context.setColored(false);
          switch (value) {
            case 2: EntryHalftone.this.context.setScatter(true); break;
            case 1: EntryHalftone.this.context.setColored(true); break;
          }
        }
  ...
  public void draw() { 
    ...
    double d = this.context.circle ? this.context.spacing * Math.sqrt(4 / Math.PI) : this.context.spacing;
    if (this.context.rounded) drawRoundedHalftone(image, d); 
    else if (this.context.scatter) {
      applet.background(255);
      applet.blendMode(SUBTRACT);
  ...
  private void drawRoundedHalftone(PImage image, double d) {
    ...
    for (double dist = 0; dist < maxDist; dist += spacing) {
      if (dist == 0)
        ...
        double round = 2 * Math.PI, da = round / Math.floor(2 * Math.PI * dist / spacing);
  ...
  private void drawElement(PImage image, double dist, double angle, double d) {
    double x = image.width / 2.0 + dist * Math.sin(angle), y = image.height / 2.0 + dist * Math.cos(angle);
    ...
    if (this.context.circle)
      this.context.ui.applet.ellipse(this.context.dx + (float)x, this.context.dy + (float)y, diameter, diameter);
    else this.context.ui.applet.rect(this.context.dx + (float)x - diameter / 2, this.context.dy + (float)y - diameter / 2, diameter, diameter);
  }
  ...
  private double getLuminance(PApplet applet, int c) {
    switch (this.context.lumType) {
      case Context.LUMTYPE_BRIGHTNESS: return applet.brightness(c);
      case Context.LUMTYPE_AVERAGE: return (applet.red(c) + applet.green(c) + applet.blue(c)) / 3.0;
      case Context.LUMTYPE_RED: return applet.red(c);
      case Context.LUMTYPE_GREEN: return applet.green(c);
      case Context.LUMTYPE_BLUE: return applet.blue(c);
      default: return 0;
    }
  }
}